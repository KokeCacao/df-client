...
function checkNumInboundVoyages(planetId, from = "") {
  if (from == "") {
    return (
      ...
      df.getUnconfirmedMoves().filter((m) => m.intent.to == planetId).length
    );
  } else {
    return (
      df
      ...
      df.getUnconfirmedMoves().filter((m) => m.intent.to == planetId && m.intent.from == from)
        .length
    );
  }
}
...
class Manager {
  ...
  distribute(minPlanetLevel = 4, silverMinPercent = 0.95) {
    const owned = df.getMyPlanets().filter((p) => p.silverGrowth > 0).filter((p) => p.silver > p.silverCap*silverMinPercent);
    let captured = [];
    owned.forEach(async (p) => {
      captured = await distributeSilver(p.locationId, 99, minPlanetLevel);
    });
  }
